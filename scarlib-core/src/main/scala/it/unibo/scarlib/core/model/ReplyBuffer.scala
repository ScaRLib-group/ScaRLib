...
object ReplyBuffer:
  def apply[S, A](size: Int): ReplyBuffer[S, A] =
    new BoundedQueue[S, A](size)

  private class BoundedQueue[State, Action](size: Int) extends ReplyBuffer[State, Action]:
...
object BufferTest extends App:
  val buffer = ReplyBuffer[Double, Int](10)
